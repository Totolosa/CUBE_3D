...
/*   Updated: 2021/02/25 17:33:15 by tdayde           ###   ########lyon.fr   */
/*                                                                            */
/* ************************************************************************** */
...
int	calcul_h_sprite(t_pars *p)
{
	...
	while (++i < p->nb_spr)
	{
		p->spr[i].v_dir = (p->spr[i].x - p->moov.x_pos) * p->moov.x_dir
			+ (p->spr[i].y - p->moov.y_pos) * p->moov.y_dir;
		p->spr[i].v_per = (p->spr[i].x - p->moov.x_pos) * (-p->moov.y_dir)
			+ (p->spr[i].y - p->moov.y_pos) * p->moov.x_dir;
		...
		p->spr[i].h_scr = (double)p->scr.d / p->spr[i].v_dir;
