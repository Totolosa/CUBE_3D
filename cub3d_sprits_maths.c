...
void init_find_sprite(t_find_sprite *res, double x, double y)
{
	res->ar = 0;
	res->br = 0;
	res->cr = 0;
...
int coord_center_sprite(t_find_sprite *res, t_ray *ray)
{
	...
	if (ray->wall.orient == 'v')
	{
		if (cos(ray->angle > 0))
			...
			res->xc -= 0.5;
		res->yc = floor(res->yc) + 0.5;
	}
	return (1);
}

int coord_contact_sprite(t_find_sprite *res, t_ray *ray, t_pars *pars)
{

	res->ar = sin(ray->angle / (180 / M_PI));
	res->br = - cos(ray->angle / (180 / M_PI));
	res->cr = cos(ray->angle / (180 / M_PI)) * pars->pos_y - sin(ray->angle / (180 / M_PI)) * pars->pos_x;
	res->as = cos(ray->angle / (180 / M_PI));
	res->bs = sin(ray->angle / (180 / M_PI));
	res->cs = - cos(ray->angle / (180 / M_PI)) * res->xc - sin(ray->angle / (180 / M_PI)) * res->yc;
	res->ys = res->cs / res->as - res->cr / res->ar;
	res->ys /= res->br / res->ar - res->bs / res->as;
	res->xs = (-res->br * res->ys - res->cr) / res->ar;
	res->len = sqrt(pow(res->xc - res->xs, 2) + pow(res->yc - res->ys, 2));
	res->vect = res->as * (res->xs - res->xc) - res->ar * (res->ys - res->yc);
	if (res->vect >= 0)
		res->xs = 
...
int add_sprite(double x, double y, t_ray *ray, t_pars *pars)
{
	...
	init_find_sprite(&res, x, y);
	coord_center_sprite(&res, ray);
	coord_contact_sprite(&res, ray, pars);


//	printf("x = %f, y = %f, nb_sprites = %d, sprites[%d].dst = %f\n", x , y, ray->nb_sprites, ray->nb_sprites, ray->sprites[ray->nb_sprites].dst);
	...
	free(ray->sprites);
	ray->sprites = new;
	ray->sprites[ray->nb_sprites].dst = sqrt(pow(y - pars->pos_y, 2) + pow(x - pars->pos_x, 2));
	ray->nb_sprites++;
	return (1);
}
...
int total_sprites(t_ray *ray, t_ray horiz, t_ray verti)
{
	...
	// {
		...
		{
//			printf("i = %d, j = %d, k = %d, horiz.nb_sprites = %d, horiz.sprites[0].dst = %f, verti.nb_sprites = %d\n", i, j, k, horiz.nb_sprites, verti.sprites[0].dst, verti.nb_sprites);
			if (horiz.nb_sprites == 0)
			{	
			...
			else if (horiz.sprites[i].dst < verti.sprites[j].dst)
			{
				ray->sprites[k] = horiz.sprites[i];
				i++;
				horiz.nb_sprites--;
...
int first_wall_horiz(t_ray *horiz, t_pars *pars, double x_dir, double y_dir)
{
			...
			((y_dir > 0 && pars->map[y][(int)x] != -1) || (y_dir < 0 && pars->map[y - 1][(int)x] != -1)))
	{
		...
		if ((y_dir > 0 && pars->map[y][(int)x] == 1) || (y_dir < 0 && pars->map[y - 1][(int)x] == 1))
		{
			horiz->wall.x = x;
			horiz->wall.y = y;
			horiz->wall.dst = sqrt(pow(horiz->wall.y - pars->pos_y, 2) + pow(horiz->wall.x - pars->pos_x, 2));
			return (1);
		}
		if (y_dir > 0)
			y += 1;
...
//	printf("x = %f, y = %d, x_dir = %f, y_dir = %f, pars->map_w = %d, pars->map_h = %d,pars->map[x][y] = %d\n", x, y, x_dir, y_dir, pars->map_w, pars->map_h,pars->map[(int)x][y]);
	...
	//	printf("Debut boucle : x = %d, y = %f,pars->map[x][y] = %d\n", x, y,pars->map[x][(int)y]);
		...
		if ((x_dir > 0 && pars->map[(int)y][x] == 1) || (x_dir < 0 && pars->map[(int)y][x - 1] == 1))
		{
			verti->wall.x = x;
			verti->wall.y = y;
			verti->wall.dst = sqrt(pow(verti->wall.y - pars->pos_y, 2) + pow(verti->wall.x - pars->pos_x, 2));
			return (1);
		}
...
//	ray->angle = fabs((pars->angle / (180 / M_PI)) - ray->angle / (180 / M_PI));
	ray->wall.dst = cos(fabs((pars->angle / (180 / M_PI)) - ray->angle / (180 / M_PI))) * ray->wall.dst;
...
//	printf("contact.dst = %f\n", contact.dst);
	wall_h_screen = (double)pars->screen_d / contact.dst;
	// if ((wall_h_screen = (double)pars->screen_d / wall.dst) % 2 == 1)
	// 	wall_h_screen += 0;
	if (wall_h_screen <= pars->screen_h)
	{
